...
def frontiere_3d(f, data, ax, step=20):
    """Plot the 3d frontier for the decision function f on given axes ax."""
    ...
    z = np.array([f(vec) for vec in np.c_[xx.ravel(), yy.ravel()]])
    z = z.reshape(xx.shape)
    ax.plot_surface(xx, yy, z, rstride=1, cstride=1, linewidth=0., antialiased=False, cmap=plt.cm.coolwarm)
...
def plot_cout3d(ax, x, y, loss_fun, w):
    """Trace the cost of the cost function loss_fun passed as a parameter, in x, y,
    varying the coordinates of the weight w.
    W can be used to pass a weight history."""
    def _inter(wn):
        ww = np.zeros(3)
        ww[1:] = wn
        return loss_fun(x, y, ww).mean()
    ...
    datarange = np.array([[w[:, 1].min(), w[:, 2].min()],
                          [w[:, 1].max(), w[:, 2].max()]])
    frontiere_3d(_inter, np.array(datarange), ax=ax)
    ax.plot(w[:, 1], w[:, 2], np.array([_inter(w[i, 1:]) for i in range(w.shape[0])]), 'k-', linewidth=3)
...
def plot_gradient(X, y, wh, cost_hist, loss_fun):
    ...
    plot_cout3d(ax3d, X, y, loss_fun, wh)
